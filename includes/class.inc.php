...
class jcore{
	...
	function cekUser($user,$pass)
	{
		...
		$q="select count(*) as j from pengguna where username='".$user."' "
		." and pass=('".$pass."') ";
		$r = $this->sql($q);
		$o = $this->fetch($r);
		if($o[0]>0){
			$_SESSION['login']=true;
			$q1="select * from pengguna where username='".$user."' "
			." and pass=('".$pass."') ";
			$r1 = $this->sql($q1);
			$o1 = $this->fetch($r1);
			$_SESSION['username']=$o1["username"];
			$_SESSION['pass']=$o1["pass"];
	...
	function readBlob($id){
		...
		$stmt = 'SELECT * FROM tbbarang';

		$sth = ibase_query($dbh, $stmt) or die(ibase_errmsg());

		/*$q = "select brgpict from tbbarang where brgid='".$id."'";
		$r = ibase_query($q);
